...
async function terminarJuego() {
    ...
    const isTop10 = await isHighScore("divisiones-teclado", currentScore);
...
async function submitHighScore() {
    ...
    await saveHighScore("divisiones-teclado", initials, currentScore, settingsStr);
...
async function showLeaderboard() {
    ...
    let scores = await getTopScores("divisiones-teclado");
    let html = "";
    if (scores.length === 0) {
...
async function loadHighestScore() {
    let top = await getTopScores("divisiones-teclado");
