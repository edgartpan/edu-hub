...
async function terminarJuego() {
    ...
    const isTop10 = await isHighScore("sumas-teclado", currentScore);
...
async function submitHighScore() {
    ...
    await saveHighScore("sumas-teclado", initials, currentScore, settingsStr);
...
async function showLeaderboard() {
    ...
    let scores = await getTopScores("sumas-teclado");
    let html = "";
    if (scores.length === 0) {
...
async function loadHighestScore() {
    let top = await getTopScores("sumas-teclado");
