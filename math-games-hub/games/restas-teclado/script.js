...
async function terminarJuego() {
    ...
    const isTop10 = await isHighScore("restas-teclado", currentScore);
...
async function submitHighScore() {
    ...
    await saveHighScore("restas-teclado", initials, currentScore, settingsStr);
...
async function showLeaderboard() {
    ...
    let scores = await getTopScores("restas-teclado");
    let html = "";
    if (scores.length === 0) {
...
async function loadHighestScore() {
    let top = await getTopScores("restas-teclado");
