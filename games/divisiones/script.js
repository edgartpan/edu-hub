...
//generate random values to divide (ensure clean division)
let targetRes = Math.floor((Math.random() * dificultad_set) + 1);
sumando2 = Math.floor((Math.random() * dificultad_set) + 1);
sumando1 = targetRes * sumando2;
...
let inc1, inc2;
let genInc = () => {
    let wrong = correcto + Math.floor(Math.random() * 5) - 2;
    if (wrong === correcto || wrong <= 0) {
        wrong = correcto + Math.floor(Math.random() * 5) + 1;
    }
    return wrong;
};
...
async function terminarJuego() {
    ...
    const isTop10 = await isHighScore("divisiones", currentScore);
...
async function submitHighScore() {
    ...
    await saveHighScore("divisiones", initials, currentScore, settingsStr);
...
async function showLeaderboard() {
    ...
    let scores = await getTopScores("divisiones");
    let html = "";
    if (scores.length === 0) {
...
async function loadHighestScore() {
    let top = await getTopScores("divisiones");
