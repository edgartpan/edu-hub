...
divsumando1.innerHTML = num1;
divsumando2.innerHTML = num2;
divresultado.innerHTML = res;

correcto = op;

let availableOps = ops.filter(o => o !== op);
availableOps.sort(() => Math.random() - 0.5);

let inc1 = availableOps[0];
let inc2 = availableOps[1];
...
async function terminarJuego() {
    ...
    const isTop10 = await isHighScore("operadores", currentScore);
...
async function submitHighScore() {
    ...
    await saveHighScore("operadores", initials, currentScore, settingsStr);
...
async function showLeaderboard() {
    ...
    let scores = await getTopScores("operadores");
    let html = "";
    if (scores.length === 0) {
...
async function loadHighestScore() {
    let top = await getTopScores("operadores");
