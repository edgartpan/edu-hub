...
let getExp = () => {
    ...
    let num1, num2, res;
    if (op === '+') {
        num1 = Math.floor((Math.random() * dificultad_set) + 1);
        num2 = Math.floor((Math.random() * dificultad_set) + 1);
        res = num1 + num2;
    } else if (op === 'x') {
        num1 = Math.floor((Math.random() * (dificultad_set/2)) + 1);
        num2 = Math.floor((Math.random() * (dificultad_set/2)) + 1);
        res = num1 * num2;
    } else if (op === '-') {
        num2 = Math.floor((Math.random() * dificultad_set) + 1);
        res = Math.floor((Math.random() * dificultad_set) + 1);
        num1 = num2 + res;
    } else {
        num2 = Math.floor((Math.random() * (dificultad_set/2)) + 1);
        res = Math.floor((Math.random() * (dificultad_set/2)) + 1);
        num1 = num2 * res;
    }
    return { str: num1 + ' ' + op + ' ' + num2, val: res };
};

let exp1 = getExp();
let exp2 = getExp();

// 20% chance to force them to be equal
if (Math.random() < 0.2) {
    exp2.val = exp1.val;
    // Just create a simple sum that matches
    exp2.str = (exp1.val - 1) + ' + 1';
}

divsumando1.innerHTML = exp1.str;
divsumando2.innerHTML = exp2.str;

if (exp1.val > exp2.val) correcto = "Izquierda";
else if (exp1.val === exp2.val) correcto = "Iguales";
else correcto = "Derecha";

btnopcion1.innerHTML = "Izquierda";
btnopcion2.innerHTML = "Iguales";
btnopcion3.innerHTML = "Derecha";
}
...
async function terminarJuego() {
    ...
    const isTop10 = await isHighScore("magnitudes", currentScore);
...
async function submitHighScore() {
    ...
    await saveHighScore("magnitudes", initials, currentScore, settingsStr);
...
async function showLeaderboard() {
    ...
    let scores = await getTopScores("magnitudes");
    let html = "";
    if (scores.length === 0) {
...
async function loadHighestScore() {
    let top = await getTopScores("magnitudes");
