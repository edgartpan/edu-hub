...
async function terminarJuego() {
    ...
    const isTop10 = await isHighScore("sumas", currentScore);
...
async function submitHighScore() {
    ...
    await saveHighScore("sumas", initials, currentScore, settingsStr);
...
async function showLeaderboard() {
    ...
    let scores = await getTopScores("sumas");
    let html = "";
    if (scores.length === 0) {
...
async function loadHighestScore() {
    let top = await getTopScores("sumas");
