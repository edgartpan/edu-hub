...
let factor1 = Math.floor((Math.random() * dificultad_set) + 1);
let factor2 = Math.floor((Math.random() * dificultad_set) + 1);
let target = factor1 * factor2;

divsumando1.innerHTML = target;

//define right and wrong answers
correcto = factor1 + " x " + factor2;

let rank = Math.floor(Math.random() * 3);

let inc1, inc2;
let genInc = () => {
    let f1 = Math.floor((Math.random() * dificultad_set) + 1);
    let f2 = Math.floor((Math.random() * dificultad_set) + 1);
    // ensure it's not the same product or exact same string
    while(f1 * f2 === target) {
        f1 = Math.floor((Math.random() * dificultad_set) + 1);
        f2 = Math.floor((Math.random() * dificultad_set) + 1);
    }
    return f1 + " x " + f2;
};
...
async function terminarJuego() {
    ...
    const isTop10 = await isHighScore("factores", currentScore);
...
async function submitHighScore() {
    ...
    await saveHighScore("factores", initials, currentScore, settingsStr);
...
async function showLeaderboard() {
    ...
    let scores = await getTopScores("factores");
    let html = "";
    if (scores.length === 0) {
...
async function loadHighestScore() {
    let top = await getTopScores("factores");
